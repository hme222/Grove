...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Welcome back to Grove');
      navigate('/');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#F5F0E8] flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-[#1C2E10] items-center justify-center relative overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden text-center mb-8">
            ...
            <p className="font-ui text-[#2B2B26] text-sm mt-1">Grow together</p>
          </div>

          <div className="rounded-[14px] border-[0.5px] border-[#D3C9B8] bg-[#EDE5D8] p-6">
            <h2 className="font-plant text-[#1C2E10] text-2xl mb-1">Welcome back</h2>
            <p className="font-ui text-[#2B2B26] text-sm mb-6">Sign in to your garden</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                className="w-full rounded-[2px] font-plant uppercase tracking-[0.08em] text-xs px-4 py-3.5 border-[0.5px] bg-[#1C2E10] text-[#F5F0E8] border-[#1C2E10] hover:bg-[#2D5016] hover:border-[#2D5016] disabled:opacity-50 transition-colors duration-150"
              >
            ...
            <div className="mt-4 text-center">
              <Link
                to="/register"
                className="font-ui text-sm text-[#3B6D11] hover:text-[#2D5016] transition-colors duration-150"
                data-testid="auth-switch-register"
              >
                New to Grove? Create an account
              </Link>
            </div>
          </div>
        </div>
